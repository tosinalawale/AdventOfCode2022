namespace AdventOfCode2022.Day14
{
    ...
    public class Day14Part01
    {
        public static int CalculateResult(string[] input)
        {
            ...
            var lowestRockYPosition = GetLowestRockPositionInGrid(grid);

            var numberOfGrainsWithARestingPosition = 0;
            (int x, int y)? restingPosition = (x: 500, y: 0);

            while (restingPosition != null)
            {
                restingPosition = DropGrainOfSand(grid, lowestRockYPosition);
        ...
        private static (int x, int y)? DropGrainOfSand(Dictionary<(int x, int y), char> grid, int lowestRockYPosition)
        {
            ...
            {
                if (!grid.ContainsKey((x: sandGrainPosition.x, y: sandGrainPosition.y + 1)))
                {
                    sandGrainPosition = (x: sandGrainPosition.x, y: sandGrainPosition.y + 1);
                }
                else if (!grid.ContainsKey((x: sandGrainPosition.x - 1, y: sandGrainPosition.y + 1)))
                {
